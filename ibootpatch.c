...
static int patch_iboot_generator(uint8_t *buf, size_t len, const char *generator)
{
    ...
    char *gen_dst = (char *)str_ref;
    memset(gen_dst, 0, strlen(needle) + 1);
    strncpy(gen_dst, generator, gen_len);
    printf("[+] wrote generator '%s' at buf+0x%llx\n", generator, (unsigned long long)str_off);

    *bl_insn = NOP;
    printf("[+] NOP'd env_get BL instruction\n");
...
int main(int argc, char *argv[])
{
    ...
    printf("surrealra1n arm64e iBoot signature & generator patcher\n");

    if (argc < 3) {
        printf("usage: %s <iBoot.in> <iBoot.patched> [-g generator]\n", argv[0]);
        return 1;
    }
    ...
    if (patch_iboot_signature_check(buf, len) != 0) {
        printf("[-] signature patching failed\n");
        free(buf);
        return 1;
    }

    if (generator) {
        if (patch_iboot_generator(buf, len, generator) != 0) {
            printf("[-] generator patching failed (continuing anyway)\n");
        }
    }
